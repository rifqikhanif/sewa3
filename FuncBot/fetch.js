...
exports.createExif = (pack, auth) =>{
    ...
    const exif = {"sticker-pack-id": "com.wann.tech", "sticker-pack-name": pack, "sticker-pack-publisher": auth, "android-app-store-link": "https://play.google.com/store/apps/details?id=com.termux", "ios-app-store-link": "https://itunes.apple.com/app/sticker-maker-studio/id1443326857"}
...
exports.modStick = (media, wann, m, from) => {
    out = getRandom('.webp')
    try {
        console.log(media)
        spawn('webpmux', ['-set','exif', './sticker/data.exif', media, '-o', out])
        .on('exit', () => {
            wann.sendMessage(from, fs.readFileSync(out), 'stickerMessage', {quoted: m})
            fs.unlinkSync(out)
            fs.unlinkSync(media)
        })
    } catch (e) {
        console.log(e)
        wann.sendMessage(from, 'Terjadi keslahan', 'conversation', { quoted: m })
        fs.unlinkSync(media)
    }
}
